...
(function() {
    ...
    window.renderSPAssessmentSetup = function() {
        ...
        // Check if we have a pre-loaded weekly SP case
        const weeklyCase = window.currentWeeklyCase;
        const hasWeeklySPCase = weeklyCase && weeklyCase.type === 'sp';
        ...
        return `
            ...
            <div class="min-h-screen bg-gradient-to-b from-green-50 to-white dark:from-zinc-900 dark:to-zinc-800 text-zinc-900 dark:text-zinc-100">
                <div class="max-w-3xl mx-auto px-6 py-12">
                    ...
                    <button onclick="goToScreen('${hasWeeklySPCase ? 'weekly-case-menu' : 'menu'}')" class="text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-100 mb-8 flex items-center gap-2">
                        ← ${hasWeeklySPCase ? 'Back to Weekly Cases' : 'Back to Menu'}
                    </button>
                    ...
                    ${hasWeeklySPCase ? `
                        <!-- Weekly SP Case Pre-loaded (Course Director) -->
                        <div class="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-2xl p-6 mb-6">
                            <h2 class="text-xl font-semibold mb-2 text-green-900 dark:text-green-100">🩺 Weekly SP Case Ready</h2>
                            <p class="text-green-700 dark:text-green-300 mb-4">
                                <strong>${weeklyCase.data.title}</strong><br>
                                Chief Complaint: "${weeklyCase.weekData.chiefComplaint}"<br>
                                Diagnosis: ${weeklyCase.data.diagnosis}
                            </p>
                            <button onclick="window.createSPSessionFromWeekly()" 
                                class="w-full py-3 bg-green-600 text-white rounded-xl hover:bg-green-700 font-semibold">
                                🚀 Create SP Session with This Case →
                            </button>
                            <p class="text-xs text-green-600 dark:text-green-400 mt-2 text-center">This will generate session codes for facilitators</p>
                        </div>
                    ` : ''}
    ...
    // Create SP session from weekly case
    window.createSPSessionFromWeekly = function() {
        const weeklyCase = window.currentWeeklyCase;
        if (!weeklyCase || weeklyCase.type !== 'sp') {
            alert('No SP case loaded');
            return;
        }
        
        // Go to session generation with the weekly case pre-loaded
        window.weeklySPCaseForSession = weeklyCase;
        goToScreen('sp-director-sessions');
    };
        ...
        return `
            ...
            <div class="min-h-screen bg-zinc-50 dark:bg-zinc-900 text-zinc-900 dark:text-zinc-100">
                ...
                <main class="max-w-6xl mx-auto px-6 py-8">
                    ...
                    ${window.weeklySPCaseForSession ? `
                        <!-- Weekly SP Case Banner -->
                        <div class="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-xl p-4 mb-6">
                            <div class="flex items-center justify-between">
                                <div>
                                    <p class="text-green-800 dark:text-green-200 font-semibold">🩺 Weekly SP Case: ${window.weeklySPCaseForSession.data.title}</p>
                                    <p class="text-green-600 dark:text-green-400 text-sm">Diagnosis: ${window.weeklySPCaseForSession.data.diagnosis}</p>
                                </div>
                                <button onclick="window.weeklySPCaseForSession = null; render();" class="text-green-600 hover:text-green-800 text-sm">Clear</button>
                            </div>
                        </div>
                    ` : ''}
                    ...
                    <div class="bg-white dark:bg-zinc-800 rounded-xl border p-6 mb-8">
                        ...
                        <div class="grid md:grid-cols-2 gap-6">
                            <div>
                                ...
                                ${window.weeklySPCaseForSession ? `
                                    <div class="w-full px-4 py-3 border rounded-lg bg-green-50 dark:bg-green-900/20 border-green-300">
                                        <p class="font-medium">${window.weeklySPCaseForSession.data.title}</p>
                                        <p class="text-sm text-green-600">(Weekly SP Case)</p>
                                    </div>
                                    <input type="hidden" id="dirCaseSelect" value="weekly-sp-${window.weeklySPCaseForSession.week}">
                                ` : `
                                ...
                                <tbody>
                                    ${sessions.map(s => `
                                        <tr class="border-b border-zinc-100">
                                            ...
                                            <td class="py-3">${s.caseType === 'weekly-sp' ? (s.caseData?.title || 'Weekly SP') : ((typeof cases !== 'undefined' ? cases.find(c => c.id === s.caseId)?.title : null) || s.caseId)}</td>
    ...
    window.spGenerateSessions = function() {
        let caseId = document.getElementById('dirCaseSelect')?.value;
        const count = parseInt(document.getElementById('dirSessionCount')?.value) || 10;
        
        // Check for weekly SP case
        const weeklyCase = window.weeklySPCaseForSession;
        const isWeeklySP = weeklyCase && caseId?.startsWith('weekly-sp-');
        
        if (!caseId && !isWeeklySP) { alert('Select a case'); return; }
        ...
        for (let i = 0; i < count; i++) {
            ...
            const sessionData = {
                code, 
                facilitatorPIN: pin, 
                caseId: isWeeklySP ? weeklyCase.data.id : caseId, 
                createdAt: new Date().toISOString(), 
                students: [], 
                studentResponses: []
            };
            
            // Add weekly case data if applicable
            if (isWeeklySP) {
                sessionData.caseType = 'weekly-sp';
                sessionData.weekKey = weeklyCase.week;
                sessionData.caseData = weeklyCase.data;
                sessionData.weekData = weeklyCase.weekData;
            }
            
            localStorage.setItem('sp_session_' + code, JSON.stringify(sessionData));
            generated.push({ code, pin, caseId: sessionData.caseId });
        }
        
        // Download CSV
        const caseTitle = isWeeklySP ? weeklyCase.data.title : ((typeof cases !== 'undefined' ? cases.find(c => c.id === caseId)?.title : null) || caseId);
        ...
        // Clear weekly case reference
        window.weeklySPCaseForSession = null;
    ...
    window.renderClassroomSetup = function() {
        ...
        // Check if we have a pre-loaded weekly case
        const weeklyCase = window.currentWeeklyCase;
        const hasWeeklyCase = weeklyCase && weeklyCase.type === 'jit';
        ...
        return `
            ...
            <div class="min-h-screen bg-gradient-to-b from-indigo-50 to-white dark:from-zinc-900 dark:to-zinc-800 text-zinc-900 dark:text-zinc-100">
                <div class="max-w-4xl mx-auto px-6 py-12">
                    ...
                    <button onclick="goToScreen('${hasWeeklyCase ? 'weekly-case-menu' : 'sp-assessment-setup'}')" class="text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-100 mb-8 flex items-center gap-2">
                        ← Back
                    </button>
                    ...
                    <div class="grid md:grid-cols-2 gap-6">
                        ...
                        <!-- How it works -->
                        <div class="bg-zinc-100 dark:bg-zinc-800/50 rounded-xl p-5">
                            <h3 class="font-semibold mb-3">How Classroom Mode Works</h3>
                            <div class="space-y-2 text-sm text-zinc-600 dark:text-zinc-400">
                                <p class="flex gap-2">📺 <span>Project instructor view on screen</span></p>
                                <p class="flex gap-2">👥 <span>Students join with session code</span></p>
                                <p class="flex gap-2">⏸️ <span>You control stage progression</span></p>
                                <p class="flex gap-2">📊 <span>Real-time word clouds & charts</span></p>
                                <p class="flex gap-2">🎲 <span>Cold call from your roster</span></p>
                                <p class="flex gap-2">🔒 <span>Responses stay anonymous</span></p>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    // Create session from weekly case
    window.createClassroomSessionFromWeekly = function() {
        const weeklyCase = window.currentWeeklyCase;
        if (!weeklyCase || weeklyCase.type !== 'jit') {
            alert('No Just-in-Time case loaded');
            return;
        }
        ...
        const session = {
            code: code,
            caseId: weeklyCase.data.id,
            caseType: 'weekly-jit',
            weekKey: weeklyCase.week,
            caseData: weeklyCase.data, // Store the full case data
            weekData: weeklyCase.weekData, // Store week data for shared differential
            mode: 'classroom',
            createdAt: new Date().toISOString(),
            currentStage: 0,
        ...
        classroomState.caseId = weeklyCase.data.id;
        classroomState.currentStage = 0;
        classroomState.currentPhase = 'individual';
        classroomState.weeklyCase = weeklyCase; // Store reference
    ...
    window.renderClassroomInstructor = function() {
        ...
        // Get case data - either from weekly case or from main cases array
        let caseData = null;
        let caseTitle = 'Case';
        let chiefComplaint = '';
        
        if (session.caseType === 'weekly-jit' && session.caseData) {
            // Weekly Just-in-Time case
            caseData = session.caseData;
            caseTitle = caseData.title;
            chiefComplaint = session.weekData?.chiefComplaint || caseData.doorNote?.chiefComplaint || '';
        } else {
            // Regular case from main cases array
            caseData = typeof cases !== 'undefined' ? cases.find(c => c.id === session.caseId) : null;
                ...
                <header class="bg-indigo-600 py-3">
                    <div class="max-w-7xl mx-auto px-6 flex justify-between items-center">
                        <div class="flex items-center gap-6">
                            <div>
                                <p class="text-indigo-200 text-xs uppercase tracking-wide">${session.caseType === 'weekly-jit' ? 'Just-in-Time Case' : 'Classroom Mode'}</p>
                                <h1 class="font-bold text-xl">${caseTitle}</h1>
                                ${chiefComplaint ? `<p class="text-indigo-200 text-sm">"${chiefComplaint}"</p>` : ''}
                            </div>
                ...
                <main class="max-w-7xl mx-auto px-6 py-6">
                    <div class="grid grid-cols-3 gap-6">
                        ...
                        <div class="col-span-2 space-y-6">
                            ...
                            <!-- Case Info Panel (for Weekly JIT) -->
                            ${session.caseType === 'weekly-jit' && session.caseData ? `
                                <div class="bg-amber-900/30 border border-amber-700 rounded-2xl p-4">
                                    <div class="flex justify-between items-start">
                                        <div>
                                            <p class="text-amber-400 text-sm font-medium">Patient: ${session.caseData.patient?.name || 'N/A'}</p>
                                            <p class="text-amber-200 text-sm">${session.caseData.patient?.demographics || ''}</p>
                                        </div>
                                        <button onclick="window.showWeeklyCaseInfo()" class="px-3 py-1 bg-amber-600 text-white rounded-lg text-sm hover:bg-amber-700">
                                            📋 Show Door Note
                                        </button>
                                    </div>
                                </div>
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="bg-zinc-800 rounded-2xl p-6">
                                ...
                                <div class="space-y-2">
                                    <button onclick="window.showWeeklyCaseInfo()" class="w-full py-2 text-left px-4 bg-zinc-700 rounded-lg hover:bg-zinc-600">
                                        📋 Show Case Info
                                    </button>
    ...
    window.showWeeklyCaseInfo = function() {
        const session = JSON.parse(localStorage.getItem('sp_session_' + classroomState.sessionCode) || '{}');
        
        if (session.caseType === 'weekly-jit' && session.caseData) {
            const caseData = session.caseData;
            const doorNote = caseData.doorNote;
            alert(`📋 DOOR NOTE\n\nLocation: ${doorNote.location}\nPatient: ${caseData.patient.demographics}\n\nChief Complaint: "${doorNote.chiefComplaint}"\n\nVitals:\nBP ${doorNote.vitals.BP} | HR ${doorNote.vitals.HR} | RR ${doorNote.vitals.RR}\nTemp ${doorNote.vitals.Temp} | SpO₂ ${doorNote.vitals.SpO2}`);
        } else {
