...
(function() {
    ...
    function autoSaveSession() {
        ...
        // Get patient info from current case/variant
        let patientName = '';
        let patientAge = '';
        let patientGender = '';
        let chiefComplaint = '';
        
        if (typeof cases !== 'undefined' && window.state.caseType) {
            const caseData = cases.find(c => c.id === window.state.caseType);
            if (caseData && caseData.variants) {
                const variantKey = window.state.variantKey || Object.keys(caseData.variants)[0];
                const variant = caseData.variants[variantKey];
                if (variant) {
                    patientName = variant.name || '';
                    patientAge = variant.age || '';
                    patientGender = variant.gender || '';
    ...
    // Learner-friendly stage name mapping
    const FRIENDLY_STAGE_NAMES = {
        'menu': 'Case Selection',
        'case-select': 'Selecting Case',
        'differential': 'Building Differential',
    ...
    function getFriendlyStage(stage) {
        if (!stage) return 'In Progress';
        return FRIENDLY_STAGE_NAMES[stage] || stage.replace(/-/g, ' ').replace(/^\w/, c => c.toUpperCase());
    }
    
    function showSessionRecoveryPrompt(session) {
        const timeAgo = formatTimeAgo(Date.now() - session.lastSave);
        
        // Build patient display string
        let patientDisplay = 'Unknown Patient';
        let chiefComplaintDisplay = '';
        
        // Try to get patient info from saved session first
        if (session.patientName) {
            const name = session.patientName;
            const age = session.patientAge || '?';
            const gender = session.patientGender ? session.patientGender.charAt(0).toUpperCase() : '?';
            patientDisplay = `${name}, ${age}${gender}`;
        } 
        // Fallback: try to get from case data
        else if (typeof cases !== 'undefined' && session.caseId) {
            const caseData = cases.find(c => c.id === session.caseId);
            if (caseData && caseData.variants) {
                const variantKey = session.variantKey || Object.keys(caseData.variants)[0];
                const variant = caseData.variants[variantKey];
                if (variant) {
                    const name = variant.name || 'Patient';
                    const age = variant.age || '?';
                    const gender = variant.gender ? variant.gender.charAt(0).toUpperCase() : '?';
                    patientDisplay = `${name}, ${age}${gender}`;
                    chiefComplaintDisplay = variant.chiefComplaint || '';
                }
            }
        }
        
        // Get chief complaint from session or fallback
        if (!chiefComplaintDisplay && session.chiefComplaint) {
            chiefComplaintDisplay = session.chiefComplaint;
        }
        
        // Truncate and clean up chief complaint for display
        if (chiefComplaintDisplay) {
            // Remove "I've" / "I'm" / "I " from start
            chiefComplaintDisplay = chiefComplaintDisplay
                .replace(/^I've /i, '')
                .replace(/^I'm /i, '')
                .replace(/^I /i, '');
            // Capitalize first letter
            chiefComplaintDisplay = chiefComplaintDisplay.charAt(0).toUpperCase() + chiefComplaintDisplay.slice(1);
            // Truncate if too long
            if (chiefComplaintDisplay.length > 60) {
                chiefComplaintDisplay = chiefComplaintDisplay.substring(0, 57) + '...';
            }
        }
        
        // Get friendly stage name
        const stageDisplay = getFriendlyStage(session.stage || session.screen);
        ...
        overlay.innerHTML = `
            <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full">
                ...
                <div class="p-6">
                    ...
                    <div class="bg-gray-50 rounded-lg p-4 mb-4">
                        <p class="font-semibold text-gray-800 text-lg">${patientDisplay}</p>
                        ${chiefComplaintDisplay ? `<p class="text-gray-600 text-sm mt-1 italic">"${chiefComplaintDisplay}"</p>` : ''}
                        <p class="text-sm text-gray-500 mt-2">📍 ${stageDisplay}</p>
                    </div>
    ...
    window.restoreRecoveredSession = function() {
        ...
        Object.assign(window.state, {
            ...
            variantKey: LR.recoveryData.variantKey || '',
