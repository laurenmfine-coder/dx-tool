...
(function() {
    ...
    // Store the original submitRevisedDifferential function from index.html
    const originalSubmitRevisedDifferential = window.submitRevisedDifferential;
    
    window.submitRevisedDifferential = function() {
        // Check which stage we're in - this function should ONLY override behavior
        // during the INITIAL differential stage for small group mode, NOT the revised-differential stage
        const currentScreen = window.state?.screen;
        
        if (currentScreen === 'revised-differential') {
            // We're at the actual revised differential stage - use the original function
            if (typeof originalSubmitRevisedDifferential === 'function') {
                originalSubmitRevisedDifferential();
            } else {
                // Fallback: set revisedSubmitted and render
                window.state.revisedSubmitted = true;
                if (typeof render === 'function') render();
            }
            return;
        }
        
        // Only apply small group mode logic during initial differential stage
        if (currentScreen === 'differential') {
            const differential = window.state?.rankedDifferential || [];
            const confidence = window.state?.initialConfidence || 50;
            ...
            // Continue to next stage (history) - this is correct for INITIAL differential
