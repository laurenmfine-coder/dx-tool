// Virtual EMR Case: ST-Elevation Myocardial Infarction (STEMI)
// Variant: stemi-v1 | Acuity: ESI-2
// Auto-generated from ED Board clinical data

window.EMR_DATA = {
  "patient": {
    "name": "Jennifer Davis",
    "dob": "08/17/1975",
    "age": 50,
    "sex": "Female",
    "mrn": "NSU-2025-92470",
    "pronouns": "She/Her",
    "insurance": "Cigna EPO",
    "pcp": "Dr. Michael Torres, MD",
    "pharmacy": "CVS Pharmacy \u2014 1340 Oak St, Hollywood, FL",
    "language": "English",
    "race": "Multiracial",
    "address": "4799 Oak St, Fort Lauderdale, FL 33537",
    "phone": "(954) 555-4264",
    "email": "j.davis17@email.com",
    "emergencyContact": {
      "name": "Linda Davis (Sister)",
      "phone": "(954) 555-4744"
    }
  },
  "problems": [
    {
      "problem": "Type 2 Diabetes Mellitus",
      "icd": "E11.65",
      "onset": "2015",
      "status": "Active",
      "notes": ""
    },
    {
      "problem": "Hypertension",
      "icd": "I10",
      "onset": "2017",
      "status": "Active",
      "notes": ""
    },
    {
      "problem": "Hyperlipidemia",
      "icd": "E78.5",
      "onset": "2020",
      "status": "Active",
      "notes": ""
    }
  ],
  "medications": [
    {
      "name": "Metformin 500mg BID",
      "sig": "Take 1 tablet by mouth twice daily with meals",
      "prescriber": "Dr. Reyes",
      "start": "01/2018",
      "refills": 1,
      "status": "Active"
    },
    {
      "name": "Lisinopril 10mg daily",
      "sig": "Take 1 tablet by mouth daily",
      "prescriber": "Dr. Reyes",
      "start": "01/2024",
      "refills": 4,
      "status": "Active"
    },
    {
      "name": "Atorvastatin 40mg QHS",
      "sig": "Take 1 tablet by mouth at bedtime",
      "prescriber": "Dr. Reyes",
      "start": "05/2019",
      "refills": 1,
      "status": "Active"
    },
    {
      "name": "Aspirin 81mg daily",
      "sig": "Take 1 tablet by mouth daily",
      "prescriber": "Dr. Reyes",
      "start": "11/2019",
      "refills": 5,
      "status": "Active"
    }
  ],
  "allergies": [
    {
      "allergen": "Penicillin",
      "type": "Drug",
      "reaction": "Rash as a child",
      "severity": "Severe",
      "verified": "Yes"
    }
  ],
  "vitals": [
    {
      "date": "01/15/2025",
      "bp": "172/102",
      "hr": 105,
      "rr": 14,
      "temp": "99.0\u00b0F",
      "spo2": "98%",
      "wt": "168 lbs",
      "ht": "5'6\"",
      "bmi": 33.9,
      "setting": "PCP Office"
    },
    {
      "date": "10/03/2024",
      "bp": "173/97",
      "hr": 107,
      "rr": 16,
      "temp": "98.4\u00b0F",
      "spo2": "96%",
      "wt": "176 lbs",
      "ht": "6'2\"",
      "bmi": 32.3,
      "setting": "PCP Office"
    },
    {
      "date": "07/22/2024",
      "bp": "173/102",
      "hr": 110,
      "rr": 18,
      "temp": "98.2\u00b0F",
      "spo2": "95%",
      "wt": "156 lbs",
      "ht": "5'10\"",
      "bmi": 23.8,
      "setting": "Urgent Care"
    },
    {
      "date": "04/10/2024",
      "bp": "155/92",
      "hr": 110,
      "rr": 18,
      "temp": "98.4\u00b0F",
      "spo2": "98%",
      "wt": "199 lbs",
      "ht": "5'4\"",
      "bmi": 27.1,
      "setting": "PCP Office"
    },
    {
      "date": "01/08/2024",
      "bp": "155/99",
      "hr": 102,
      "rr": 20,
      "temp": "98.6\u00b0F",
      "spo2": "97%",
      "wt": "165 lbs",
      "ht": "5'6\"",
      "bmi": 29.8,
      "setting": "PCP Office"
    }
  ],
  "visits": [
    {
      "id": "V001",
      "date": "01/15/2025",
      "type": "Primary Care",
      "provider": "Dr. Michael Torres, MD",
      "cc": "Follow-up: Type 2 Diabetes Mellitus (10 years), Hypertension (8 years),",
      "hpi": "50-year-old female presenting for routine follow-up of Type 2 Diabetes Mellitus (10 years), Hypertension (8 years), Hyperlipidemia (5 years). Patient reports compliance with medications. No new complaints. Denies chest pain, shortness of breath, or weight changes.",
      "exam": "General: Well-appearing female in NAD. HEENT: PERRL, EOMI, oropharynx clear. Neck: Supple, no LAD. CV: RRR, no murmurs. Lungs: CTAB. Abdomen: Soft, NT/ND. Extremities: No edema. Neuro: A&O x3.",
      "assessment": "1. Type 2 Diabetes Mellitus (10 years) \u2014 stable on current regimen\n2. Hypertension (8 years) \u2014 stable on current regimen\n3. Hyperlipidemia (5 years) \u2014 stable on current regimen",
      "plan": "1. Continue current medications; f/u 3 months\n2. Continue current medications; f/u 3 months\n3. Continue current medications; f/u 3 months"
    },
    {
      "id": "V002",
      "date": "09/12/2024",
      "type": "Primary Care",
      "provider": "Dr. Michael Torres, MD",
      "cc": "Annual wellness visit",
      "hpi": "50-year-old female for annual wellness exam. Medication adherence reported. No new complaints. Screening up to date. Exercise 2-3x/week.",
      "exam": "General: NAD. Vitals: WNL. HEENT: WNL. CV: RRR. Lungs: CTAB. Abdomen: Soft, NT. Skin: No suspicious lesions.",
      "assessment": "1. Annual wellness exam\n2. Chronic conditions stable (Type 2 Diabetes Mellitus (10 years), Hypertension (8 years))",
      "plan": "1. Continue current medications\n2. Age-appropriate screening current\n3. RTC 3-6 months or PRN"
    }
  ],
  "labs": [
    {
      "date": "01/15/2025",
      "time": "08:32",
      "orderedBy": "Dr. Michael Torres, MD",
      "collected": "01/15/2025 07:45",
      "facility": "NSU Health \u2014 Davie Campus Lab",
      "accession": "LAB-2025-069533",
      "status": "Final",
      "specimenType": "Serum, Whole Blood (EDTA)",
      "fasting": "Yes (10 hrs)",
      "groups": [
        {
          "name": "COMPREHENSIVE METABOLIC PANEL (CMP)",
          "results": [
            {
              "test": "Glucose",
              "value": "166",
              "unit": "mg/dL",
              "range": "70-100",
              "flag": "H"
            },
            {
              "test": "BUN",
              "value": "21",
              "unit": "mg/dL",
              "range": "7-20",
              "flag": ""
            },
            {
              "test": "Creatinine",
              "value": "0.7",
              "unit": "mg/dL",
              "range": "0.6-1.2",
              "flag": ""
            },
            {
              "test": "Sodium",
              "value": "138",
              "unit": "mEq/L",
              "range": "136-145",
              "flag": ""
            },
            {
              "test": "Potassium",
              "value": "4.2",
              "unit": "mEq/L",
              "range": "3.5-5.0",
              "flag": ""
            },
            {
              "test": "Chloride",
              "value": "102",
              "unit": "mEq/L",
              "range": "98-106",
              "flag": ""
            },
            {
              "test": "CO2 (Bicarbonate)",
              "value": "24",
              "unit": "mEq/L",
              "range": "23-29",
              "flag": ""
            },
            {
              "test": "Calcium",
              "value": "9.1",
              "unit": "mg/dL",
              "range": "8.5-10.5",
              "flag": ""
            },
            {
              "test": "ALT (SGPT)",
              "value": "26",
              "unit": "U/L",
              "range": "7-56",
              "flag": ""
            },
            {
              "test": "AST (SGOT)",
              "value": "23",
              "unit": "U/L",
              "range": "10-40",
              "flag": ""
            }
          ]
        },
        {
          "name": "HEMOGLOBIN A1c",
          "results": [
            {
              "test": "HbA1c",
              "value": "7.4",
              "unit": "%",
              "range": "<5.7 normal; 5.7-6.4 prediabetes",
              "flag": "H"
            },
            {
              "test": "Est. Avg Glucose (eAG)",
              "value": "165",
              "unit": "mg/dL",
              "range": "\u2014",
              "flag": ""
            }
          ]
        },
        {
          "name": "CBC WITH DIFFERENTIAL",
          "results": [
            {
              "test": "WBC",
              "value": "7.6",
              "unit": "x10\u00b3/\u00b5L",
              "range": "4.5-11.0",
              "flag": ""
            },
            {
              "test": "Hemoglobin",
              "value": "11.9",
              "unit": "g/dL",
              "range": "12.0-16.0",
              "flag": ""
            },
            {
              "test": "Hematocrit",
              "value": "35.7",
              "unit": "%",
              "range": "36.0-46.0",
              "flag": ""
            },
            {
              "test": "Platelet Count",
              "value": "269",
              "unit": "x10\u00b3/\u00b5L",
              "range": "150-400",
              "flag": ""
            }
          ]
        }
      ]
    }
  ],
  "imaging": [
    {
      "date": "01/08/2024",
      "study": "CHEST XR PA AND LATERAL",
      "accession": "IMG-2024-01081",
      "status": "FINAL",
      "orderedBy": "Dr. Michael Torres, MD",
      "readBy": "Dr. James Chen, MD (Radiology)",
      "facility": "NSU Health \u2014 Davie Campus Imaging",
      "priority": "Routine",
      "clinical": "50F routine evaluation. PMH: Type 2 Diabetes Mellitus (10 years), Hypertension (8 years).",
      "technique": "PA and lateral views, upright, full inspiration.",
      "findings": "Heart: Normal size. No pericardial effusion.\\n\\nMediastinum: Normal contours.\\n\\nLungs: Clear bilaterally. No consolidation, mass, effusion, or pneumothorax.\\n\\nBones: No acute fracture.",
      "impression": "1. Normal chest radiograph.\\n2. No acute cardiopulmonary process.",
      "dictated": "01/08/2024 10:44",
      "verified": "01/08/2024 13:18"
    }
  ],
  "immunizations": [
    {
      "vaccine": "Influenza (Quadrivalent)",
      "date": "09/12/2024",
      "site": "Left deltoid IM",
      "lot": "FL2024-208",
      "mfr": "Sanofi Pasteur"
    },
    {
      "vaccine": "COVID-19 (Pfizer 2024-25)",
      "date": "10/15/2024",
      "site": "Right deltoid IM",
      "lot": "CV2024-995",
      "mfr": "Pfizer"
    },
    {
      "vaccine": "Tdap (Boostrix)",
      "date": "2019",
      "site": "Left deltoid IM",
      "lot": "\u2014",
      "mfr": "GSK"
    },
    {
      "vaccine": "Shingrix (1/2)",
      "date": "06/2023",
      "site": "Left deltoid IM",
      "lot": "SX23-441",
      "mfr": "GSK"
    },
    {
      "vaccine": "Shingrix (2/2)",
      "date": "08/2023",
      "site": "Left deltoid IM",
      "lot": "SX23-607",
      "mfr": "GSK"
    }
  ],
  "familyHistory": [
    "Mother: MI at age 58 (deceased)",
    "Father: T2DM, alive at 78",
    "Sister: HTN, age 55"
  ],
  "socialHistory": [
    [
      "Occupation",
      "Teacher"
    ],
    [
      "Marital",
      "Married"
    ],
    [
      "Tobacco",
      "Never smoker"
    ],
    [
      "Alcohol",
      "Occasional wine, 1-2/week"
    ],
    [
      "Drugs",
      "Denies"
    ],
    [
      "Exercise",
      "2-3x/week"
    ],
    [
      "Housing",
      "Lives with spouse"
    ],
    [
      "Safety",
      "Denies IPV; seatbelt"
    ],
    [
      "Advance Directive",
      "None \u2014 discussed"
    ]
  ],
  "meta": {
    "caseId": "stemi-v1",
    "diagnosis": "ST-Elevation Myocardial Infarction (STEMI)",
    "acuity": 2,
    "presentation": "Chest Pain",
    "category": "cardiovascular"
  }
};
