...
window.EMR_DATA = {
  ...
  "guided": {
    ...
    "coachPrompts": {
      ...
      "final": "Diagnosis: community-acquired pneumonia (CAP) — a common diagnosis with evolving guidelines and a wide severity spectrum. Key learning: (1) Diagnosis of CAP requires clinical features (cough, fever, sputum production, pleuritic chest pain, dyspnea) plus radiographic evidence of infiltrate. Chest X-ray is the standard initial imaging; chest CT is more sensitive for subtle disease and may change management in equivocal cases. Atypical presentations are common in the elderly (confusion, falls, anorexia, generalized weakness — sometimes without fever or cough) and immunocompromised patients. (2) Microbiology and empiric coverage. Typical pathogens: Streptococcus pneumoniae (still most common), Haemophilus influenzae, Moraxella catarrhalis, Staphylococcus aureus (including MRSA in risk settings). Atypical pathogens: Mycoplasma pneumoniae, Chlamydia pneumoniae, Legionella. Viral: influenza, RSV, SARS-CoV-2, other respiratory viruses. In most CAP, specific microbiologic diagnosis is not obtained — empiric therapy is based on severity and risk factors. Sputum culture, blood cultures, urine antigen (S. pneumoniae, Legionella), respiratory viral panel, and procalcitonin are useful in severe cases, atypical presentations, or when failing therapy. (3) Severity assessment. CURB-65 (clinical score, 0-5): admission generally recommended at >=2, ICU consideration at >=3. PSI (Pneumonia Severity Index): more granular 5-class stratification, commonly used in emergency settings. ATS/IDSA severe CAP criteria trigger ICU admission (at least 1 major or 3 minor criteria — see earlier case). Biomarkers: lactate (sepsis severity), procalcitonin (bacterial vs viral guidance, antibiotic de-escalation). (4) Empiric antibiotic selection (2019 ATS/IDSA guidelines). Outpatient, healthy, no comorbidities: amoxicillin OR doxycycline OR macrolide (if local pneumococcal resistance <25%). Outpatient with comorbidities (chronic heart/lung/liver/renal disease, diabetes, alcohol use disorder, malignancy, asplenia): beta-lactam (amoxicillin-clavulanate or cephalosporin) plus macrolide or doxycycline; OR respiratory fluoroquinolone monotherapy. Inpatient non-severe: beta-lactam plus macrolide; OR respiratory fluoroquinolone. Inpatient severe: beta-lactam plus macrolide; OR beta-lactam plus respiratory fluoroquinolone. Add empiric MRSA (vancomycin/linezolid) and Pseudomonas (piperacillin-tazobactam, cefepime, meropenem) coverage based on risk factors: prior MRSA or Pseudomonas respiratory isolate (strongest predictor), recent hospitalization with parenteral antibiotics in last 90 days. Duration typically 5-7 days; guided by clinical response and procalcitonin trends; extend for complicated pneumonia (empyema, abscess, bacteremia, specific organisms like Legionella, MRSA, Pseudomonas). (5) Prevention is high-yield. Vaccination: PCV20 or PCV15+PPSV23 sequence for older adults and selected younger adults with risk conditions; annual influenza vaccination; RSV vaccine for adults over 60; SARS-CoV-2 vaccines and boosters per current guidance. Smoking cessation. Oral hygiene (reduces aspiration pneumonia). Classic pitfalls: (a) 'healthcare-associated pneumonia' (HCAP) is no longer a category in 2019 guidelines — empiric broad coverage based solely on nursing home residence has been abandoned; instead individualize based on specific MRSA and Pseudomonas risk factors. (b) Missing alternative diagnoses that mimic pneumonia: PE, heart failure, lung cancer, aspiration, COVID-19, tuberculosis, fungal pneumonia in immunocompromised. (c) Over-prescribing antibiotics for viral illnesses — procalcitonin and viral testing help when available. (d) Not following up CXR at 6-8 weeks post-treatment in older adults or smokers — persistent infiltrate may unmask underlying lung cancer or non-infectious cause."
    }
  }
};
