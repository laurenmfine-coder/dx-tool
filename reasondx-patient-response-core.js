...
const PatientResponseEngine = (function() {
    ...
    const questionPatterns = {
        ...
        priorEpisodes: {
            keywords: ['before', 'previous', 'prior', 'ever', 'first time', 'happened before', 'history of'],
            phrases: ['ever had this before', 'happened before', 'have you ever', 'is this the first', 'first time', 'had this before', 'previous episodes', 'similar before'],
            priority: 1
        },
...
const PatientBackgroundLibrary = (function() {
    ...
    // --------------------------------------------------
    // HPI RESPONSES (for OLDCARTS questions)
    // These provide fallback answers when case doesn't have specific ones
    // --------------------------------------------------
    const hpiResponses = {
        onset: {
            acute: [
                "It just started a few hours ago.",
                "This came on suddenly, maybe 2-3 hours ago.",
            ...
            subacute: [
                ...
                "A few days. Maybe started on Monday.",
                "It's been a couple days."
            ],
            chronic: [
                "It's been going on for weeks now.",
                "This has been happening for a while - maybe a month.",
                "Several weeks at least.",
                "It's been gradual, over the past few weeks."
            ]
        },
        duration: {
            brief: [
                "Each episode lasts just a few minutes.",
                "It comes and goes - maybe 5-10 minutes at a time.",
                "Brief episodes, few minutes each."
            ],
            prolonged: [
                "Once it starts, it lasts for hours.",
                "It's pretty constant throughout the day.",
                "It doesn't really go away, just varies in intensity."
            ],
            constant: [
                "It's been constant since it started.",
                "It hasn't let up - it's there all the time.",
        ...
        priorEpisodes: {
            firstTime: [
                ...
                "First time. That's why I'm worried."
            ],
            recurrent: [
            ...
            chronic: [
                ...
                "It's a chronic thing, but this episode is worse."
            ]
        },
        severity: {
            mild: [
                "It's maybe a 3 or 4 out of 10.",
                "Not terrible - maybe a 3.",
                "Mild to moderate, I'd say a 4."
            ],
            moderate: [
                "About a 5 or 6 out of 10.",
                "Moderate - I'd say a 6.",
                "It's a 5 - definitely noticeable."
            ],
            severe: [
                "It's pretty bad - 7 or 8 out of 10.",
                "Really bad, maybe an 8.",
                "Severe - I'd say 8 out of 10."
            ],
            worst: [
        ...
        aggravating: {
            ...
            eating: [
                ...
                "Food seems to trigger it."
            ],
            nothing: [
                "Nothing specific makes it worse.",
                "It's just constant - nothing seems to change it.",
        ...
        alleviating: {
            rest: [
                ...
                "It's better when I sit still.",
                "Rest seems to help."
            ],
            medication: [
                "I took some Tylenol/ibuprofen and it helped a bit.",
                "Pain medicine takes the edge off.",
            ...
            position: [
                ...
                "It's better in certain positions.",
                "Sitting up/leaning forward helps."
            ]
        },
        radiation: {
            none: [
            ...
            yes: [
                "Yes, it spreads to my back/arm/leg.",
                "I feel it in other areas too.",
                "It radiates from here to here."
            ]
        },
        associated: {
            none: [
                "No, just this one symptom.",
                "Nothing else that I've noticed.",
                "This is the main thing."
            ],
            some: [
                "I've also been feeling nauseated.",
                "I've had some other symptoms too.",
    ...
    function getRandomResponse(category, subcategory) {
        const categoryMap = {
            ...
            livingSituation, sexualHistory, travel, rosNegatives, hpiResponses
        };
    ...
    /**
     * Get HPI response (onset, duration, prior episodes, etc.)
     */
    function getHpiResponse(questionType, variant) {
        const hpiCategory = hpiResponses[questionType];
        if (!hpiCategory) return null;
        
        // Try to pick appropriate subcategory based on variant/context
        // Default to first available subcategory
        const subcategories = Object.keys(hpiCategory);
        const defaultSubcat = subcategories[Math.floor(Math.random() * subcategories.length)];
        
        const responses = hpiCategory[defaultSubcat];
        if (!responses || !Array.isArray(responses)) return null;
...
const ReasonDxResponseIntegration = (function() {
    ...
    function enhancedMatch(studentQuestion, caseHistoryQuestions, variant) {
        ...
        const fallback = generateFallback(studentQuestion, profile, variant);
        ...
        if (fallback) {
            return {
                ...
                tier: fallback.isHpi ? 'hpi' : 'background',
                linkedDx: [],
                reasoning: fallback.isHpi ? 'HPI question' : 'Background information',
                isGenerated: true
            };
        }
     ...
     */
    function generateFallback(question, profile, variant) {
        const match = PatientResponseEngine.categorizeQuestion(question);
        if (!match || match.confidence < 2) return null;
        
        // HPI question categories that need special handling
        const hpiCategories = ['onset', 'duration', 'priorEpisodes', 'severity', 'aggravating', 'alleviating', 'radiation', 'associated', 'location', 'quality', 'timing'];
        
        // Check if this is an HPI question
        if (hpiCategories.includes(match.category)) {
            const response = PatientBackgroundLibrary.getHpiResponse(match.category, variant);
            if (response) {
                return {
