// CoachDx AI Attending - Cloudflare Worker
// Supports CoachDx (Socratic attending) + generic passthrough for SimRoomDx & other modules
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      // Route by type
      if (type === 'passthrough') {
        return handlePassthrough(body, env);
      } else {
        // Default: original CoachDx attending behavior
        return handleCoachDx(body, env);
      }

    } catch (error) {
      console.error('Worker error:', error);
      return corsJson({ error: 'Internal server error', message: error.message }, 500);
    }
  },
};

// ─────────────────────────────────────────────
//  GENERIC PASSTHROUGH (SimRoomDx, Virtual Hospital, etc.)
//  Client sends: { type:'passthrough', model, max_tokens, messages, system }
// ─────────────────────────────────────────────
async function handlePassthrough(body, env) {
  ...
  const apiBody = {
    ...
    max_tokens: Math.min(max_tokens || 400, 1024),
    messages,
  };
  if (system) {
  ...
  if (!response.ok) {
    ...
    return corsJson({ error: 'AI service error', details: data }, response.status);
  }
...
// ─────────────────────────────────────────────
//  COACHDX ATTENDING (original behavior, unchanged)
// ─────────────────────────────────────────────
async function handleCoachDx(body, env) {
...
Remember: Only reference what has ACTUALLY been said in this conversation.`;
  ...
  if (!response.ok) {
    ...
    return corsJson({ error: 'AI service error', details: error }, 500);
  }
  ...
  return corsJson({ response: aiResponse, questionType: questionType });
}
