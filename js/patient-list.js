  ...
  window.PatientList = {
    ...
    render: function(setting, specialty, embedded) {
      ...
      if (!embedded) {
        ...
        html += '<div style="display:flex;align-items:center;gap:10px"><span style="font-size:24px">' + settingEmoji + '</span><h1 style="font-size:24px;font-weight:700;color:#1A1A2E;margin:0">Patient List</h1><span style="padding:4px 12px;border-radius:6px;font-size:12px;font-weight:600;color:#2874A6;background:#EBF5FB">' + _esc(settingLabel) + '</span></div>';
        html += '<button type="button" onclick="PatientList._toggleSpoiler()" style="padding:8px 16px;border-radius:8px;font-size:12px;font-weight:600;font-family:inherit;cursor:pointer;border:1px solid ' + (_filter.hideDx ? '#2874A6' : '#DFE1E6') + ';background:' + (_filter.hideDx ? '#EBF5FB' : '#fff') + ';color:' + (_filter.hideDx ? '#2874A6' : '#5A6178') + '">' + (_filter.hideDx ? '\uD83D\uDE48 Diagnoses Hidden' : '\uD83D\uDC41\uFE0F Hide Diagnoses') + '</button>';
        html += '</div>';
      } else {
        html += '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px"><span style="font-size:13px;color:#5A6178">' + filtered.length + ' of ' + cases.length + ' patients</span>';
        html += '<button type="button" onclick="PatientList._toggleSpoiler()" style="padding:4px 10px;border-radius:6px;font-size:11px;font-weight:600;font-family:inherit;cursor:pointer;border:1px solid ' + (_filter.hideDx ? '#2874A6' : '#DFE1E6') + ';background:' + (_filter.hideDx ? '#EBF5FB' : '#fff') + ';color:' + (_filter.hideDx ? '#2874A6' : '#8C92A4') + '">' + (_filter.hideDx ? '\uD83D\uDE48 Hidden' : '\uD83D\uDC41\uFE0F Hide Dx') + '</button></div>';
      }
