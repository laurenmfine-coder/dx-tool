...
(function() {
    ...
    window.submitIndividualDifferential = function() {
        // Only run if Small Group Mode is actually enabled
        if (!SGM.enabled) {
            console.log('[SGM] submitIndividualDifferential called but SGM not enabled, ignoring');
            return;
        }
    ...
    window.submitRevisedDifferential = function() {
        // Only run if Small Group Mode is actually enabled
        if (!SGM.enabled) {
            console.log('[SGM] submitRevisedDifferential called but SGM not enabled, ignoring');
            return;
        }
        ...
        // Continue to next stage - only if we're actually in differential stage
        if (typeof advanceToHistory === 'function' && window.state?.screen === 'differential') {
            advanceToHistory();
        }
    };
