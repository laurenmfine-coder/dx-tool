...
(function() {
  ...
  function insert() {
    ...
    // Remove old inline .nav if it's the first child and matches old pattern
    var firstChild = document.body.firstElementChild;
    if (firstChild && firstChild.tagName === 'NAV' && firstChild.classList.contains('nav') && !firstChild.classList.contains('rdx-nav')) {
      // Keep the old nav's space but it will be visually replaced
    }
